# **The 20-Week Integrated Strength Program (V26.9: Longevity Update) \- Fully Expanded**

## **Global Protocols (Reference)**

**The "Cold Start" Rower Protocol (Perform before Day 1 & 5\)**

* Minute 1–2: Easy Rower (Zone 1\)
* Minute 3–4: Moderate Rower (Zone 2\)
* Minute 5: 3 x 10s Sprints (Wake up CNS)

**Global Tendon Prep (Perform immediately after Rower)**

* 20 Band Pull-Aparts
* 15 Band External Rotations (per side)
* 3 x 5 Scapular Pull-ups with 3s ISO-HOLD at top (No bouncy reps)
* 20–30s Passive Bar Hang

**Straight-Arm Band Pulldown Progression (Perform after Tendon Prep on Day 1 & Day 5\)**

**Progression Schedule (4-week cycle, repeat with band upgrade or load increase):**

* **Week 1 (+ W5, W9, W13, W17):** 3×12 @ easy band, RPE 6
* **Week 2 (+ W6, W10, W14, W18):** 3×15 @ same band, RPE 6–7
* **Week 3 (+ W7, W11, W15, W19):** 4×12 @ same band, RPE 7
* **Week 4 (+ W8, W12, W16, W20):** 2–3×12 (deload), RPE 5–6

**Setup & Execution:**

* Neutral/shoulder-width grip on band, slight hip hinge, ribs down
* Tempo: 2-1-2-1 (down–pause–up–pause)
* Lead with lats/scaps; finish with scapular depression
* Stop well before elbow irritation; if elbows complain, drop 20–30% volume that week
* If elbows feel sore post-session, substitute with light chest-supported rows for that week

## **Program Rules (Reference)**

* **Weekly Pull Volume Cap:** 36–40 total pull-up reps/week.
* **Early Warning System:** If any trigger occurs, reduce volume by \~30% for the week.
  * Elbow tightness \> 24 hours
  * Sharp pain during Ring Rows or PPPU
  * Grip fatigue lasting into Day 3
  * Shoulder pinch/impingement on Dips or HSPU
* **Rep Before Load (Double Progression):** Add weight only after hitting the top rep range for all sets with perfect form.
* **Standard Tempo:** 2-1-1-0 unless specified.
* **Day 5 Accessory Cap:** 10-minute hard cap.
* **Dip Governor:** If shoulder pinch occurs during weighted dips, switch to Straight Bar Dips or Weighted Bench Dips.
* **Grip Saver:** Day 5 core work is ground-based (no hanging core) to reduce medial epicondylitis risk.
* **Neutral Grip Tracking (Block 3):** Use the same neutral grip implement for the entire block.

## **Day 5 Specialized Routine: Load-Aware Programming (Strategic Framework)**

**Context:** Day 5 is the final strength session of the week—it arrives 48 hours after Day 1's high-volume pulling and 24 hours after Day 3's power cleans + posterior chain work. By Day 5, your lats, grip, and elbows are already stressed. This section outlines how to manage Day 5 intensity and volume across all blocks.

### **Pre-Workout Decision Tree (All Blocks)**

Use this framework before EVERY Day 5 session to avoid overuse injuries:

**GREEN LIGHT (Proceed as Written):**
- ✅ Elbows feel normal (no lingering soreness)
- ✅ Grip strength feels recovered
- ✅ No shoulder pinching during warm-up dips
- ✅ Lower back feels loose (not tight from Day 3)

**YELLOW LIGHT (Reduce Volume by 20-30%):**
- ⚠️ Mild elbow tightness (not painful, just "aware")
- ⚠️ Grip feels tired but functional
- ⚠️ Shoulders feel "cranky" but not pinching
- ⚠️ Lower back slightly stiff but improves with movement

**Modification:** Drop 1 set from pull-ups, reduce dip reps by 2 per set, reduce accessory to 6-minute cap

**RED LIGHT (Modify or Skip):**
- 🛑 Elbow pain at rest or during warm-up
- 🛑 Grip can't close tightly
- 🛑 Sharp shoulder pain during dips
- 🛑 Lower back pain radiating or limiting movement

**Modification Options:**
1. **Skip weighted pull-ups** → Substitute: Chest-Supported Rows 3 × 10
2. **Skip dips** → Substitute: Landmine Press or Push-Ups
3. **Skip accessories entirely** → Extra mobility work instead
4. **Full rest day** → Repeat Day 2 recovery protocol

### **Cumulative Fatigue by Day 5 (Weekly Context)**

| Training Stimulus | Day 1 Impact | Day 3 Impact | Day 5 Consequence |
|---|---|---|---|
| **Pull-Up Volume** | 24–36 reps | — | Lats + biceps heavily fatigued |
| **Power Cleans** | — | 4 × 3 @ moderate load | Grip demand + lat engagement already high |
| **Posterior Chain** | — | RDL + rows + leg curls | Lower back fatigue carries over |
| **Cumulative Effect** | Tendon stress starting | Tendon stress peaking | HIGH INJURY RISK if ignoring signals |

### **Block-Specific Accessory Caps (Replace 10-Min Standard)**

| Block | Weeks | Day 1 Volume Context | Day 5 Accessory Cap | Rationale |
|---|---|---|---|---|
| 1 | 1-4 | 24–30 reps | **8 min** | Conservative: building tolerance |
| 2 | 5-8 | 30–36 reps (EMOM peak W6) | **7–10 min** (reduce to 7 W7) | W6 peak: single heavy set; W7 reduce cap |
| 3 | 9-12 | 36+ reps (clusters) | **8 min** | Neutral grip phase: tendon protection priority |
| 4 | 13-16 | Variable (autoregulated) | **5 min** | Peak strength: minimal volume, quality only |
| 5 | 17-21 | 12–20 reps (taper) | **0–6 min** (mobility instead) | Taper: gradual deload, longevity focus |

### **Nutrition & Recovery Optimization for Day 5**

**Pre-Workout (60-90 min before):**
- 30–40g carbs (rice cakes, banana, dates)
- 10–15g protein (protein shake or Greek yogurt)
- Caffeine if tolerated (100–200mg) to enhance neural drive

**Post-Workout (Within 30 min):**
- 30–40g protein (whey or whole food)
- 40–60g carbs (rice, potatoes, fruit)
- Sodium replacement (especially after Day 3 power cleans)

**Sleep Priority (Night After Day 5):**
- Target 8–9 hours (higher than usual 7–8)
- Keep room cool (66–68°F / 19–20°C)
- Magnesium glycinate (400mg) if available
- Avoid alcohol (disrupts REM and recovery)

### **Common Day 5 Mistakes to Avoid**

| Mistake | Why It's Wrong | Correct Approach |
|---|---|---|
| Making up volume from Day 1 | Day 1 = VOLUME, Day 5 = INTENSITY. Adding volume defeats the split. | Accept Day 1 performance as-is. Day 5 focuses on heavy loads, low volume. |
| Ignoring elbow warning signs | Cumulative tendon stress from Days 1 + 3 = vulnerable elbows by Day 5. | Use RED LIGHT protocol immediately. One skipped Day 5 > 4 weeks of tendinitis. |
| Treating accessories as "extra credit" | Accessories should support main lifts, not become conditioning. | Respect the time cap. Finish early? Do MORE mobility instead. |
| Skipping cool-down because feeling fine | Day 5 ends the training week. Poor cool-down compounds into Day 1 of next week. | Minimum 5-minute cool-down (mandatory). |
| Not adjusting for Week 7 peak | Week 7 is highest intensity. Standard Day 5 may be too much. | Proactively reduce accessory cap to 6–7 minutes on Week 7 of each block. |

## **Power Clean Integration (Day 3 \- New)**

Power Cleans are integrated on Day 3 (Posterior Chain Day) to develop explosive hip extension, lat-posterior chain timing, and dynamic pulling power without hypertrophy fatigue. **Current working top single \= 87 kg.**

### **Blocks 1–2 (Weeks 1–8): Technique \+ Speed Emphasis**

* **Prescription:** 4 sets × 3 reps @ **52–60 kg** (60–70% of 87 kg)
* **Goal:** Smooth pulls, vertical bar path, fast turnover, no grinding
* **RPE:** 6–7 max (never grind for reps)
* **Progression:** Add 2.5 kg **only** when all reps are crisp and perfect form maintained
* **Week 4 & 8 (Deload):** Reduce to 3 × 3 @ 40–50 kg, light technique focus

### **Block 3 (Weeks 9–12): Power \+ Consistency**

* **Prescription:** 5 sets × 2 reps @ **60–65 kg** (70–75% of 87 kg)
* **Goal:** Fast bar speed, stable rack position, no early arm pull
* **Critical Rule:** If elbows feel irritated → skip cleans that week; substitute with Clean Pulls or High Pulls
* **Week 12 (Deload):** Reduce to 2 × 2 @ 30–40 kg, unweighted or very light

### **Block 4 (Weeks 13–16): Strength-Speed (Not Maxing)**

* **Prescription:** Cluster doubles: **3 × (2-2)** @ **65–70 kg** (75–80% of 87 kg)
  * 15–20s rest between each rep within the cluster
  * Full 2–3 minute recovery between sets
* **Goal:** Quality exposures with neural intent, zero missed reps, stable catches
* **Week 16 (Deload):** Switch to **Clean Pulls** @ 70–85 kg (build pulling strength without catching impact)

### **Block 5 (Weeks 17–21): Taper**

* **Weeks 17–19:** Light Hang Power Cleans
  * Week 17: 3 × 2 @ 35–45 kg, RPE 5 max
  * Week 18: 2 × 2 @ 30–40 kg (minimal power work)
  * Week 19: 2 × 1 @ 40–50 kg (light singles, RPE 4\)
* **Weeks 20–21:** Hang High Pulls only (2 × 3 @ 40–55 kg) — no catching impact

### **Power Clean Warm-Up (Before Day 3 work)**

1. Bar only: 5 Muscle Clean Pulls
2. Bar only: 5 Hang High Pulls
3. Bar only: 5 Hang Power Clean \+ Front Rack Pause
4. 3 × 3 @ 40–50% (20–30 kg) — movement rehearsal
5. Progress to working sets (no jumps \> 5 kg between sets)

### **Injury Prevention Guardrails**

Immediately reduce or skip cleans if you notice:

* Elbow traction soreness post-session
* Wrist compression in rack position
* Medial knee irritation on receiving
* Grip fatigue affecting pull-up quality

**Substitutions on high-stress weeks:**

* Week A (scheduled cleans) → Week B (High Pulls \+ Clean Pulls instead)
* Or reduce cleans to light technique singles (RPE 4–5)

**Ankle Mobility Circuit (Perform on Day 2\)**

* Knee-to-Wall Mobilization: 2 mins/side
* Tibialis Wall Raises: 3 sets of 15–20 reps
* Deep Squat Iso-Hold: 2 sets of 30s

# **BLOCK 1: FOUNDATION (Weeks 1–4)**

**Block Objectives:** Establish volume baseline (36–40 reps), reinforce squat patterns, build tissue tolerance.

## **WEEK 1**

**Summary:** Establishing the volume baseline with moderate intensity.

### **Day 1: Pull Volume (Workout A)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:**
  * 20 Band Pull-Aparts
  * 15 Band External Rotations (per side)
  * 3 x 5 Scapular Pull-ups (3s Hold)
  * 20–30s Passive Bar Hang
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).
* **Skill Practice (Crow Stand):** 3 sets of 10–20 seconds.

**The Workout**

* **A1: Pull-Up Ladders:** 4 sets of (1, 2, 3 reps) – Total 24 reps.
* **B1: Ring Rows:** 3 sets of 10 reps (Tempo 2-1-1-0).
* **B2: Incline Push-Ups:** 3 sets of 10 reps (Tempo 2-1-1-0).
* **C1: Hollow Rocks:** 3 sets of 20–25 seconds.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s (Spinal decompression).
* **Butcher’s Block Stretch:** 60s (Targeting Lats/Triceps).
* **Cobra Stretch:** 60s (Releasing abs).
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Anti-Rotation \+ Ankles (Revised Standard Template)**

**Ankle & Gait Prep (Perform First):**

* Knee-to-Wall Mobilization — 2–3 min/side
* Tibialis Raises (Wall) — 3 × 15–20
* Deep Supported Squat Iso-Hold — 2 × 30–45s

**Hip Capsule & Rotation Control:**

* 90/90 Transitions — 2 × 8/side
* Standing Hip CARs — 2 × 5/side

**Core (Stability Emphasis):**

* Pallof Press — 3 × 12/side
* Side Plank (Knee-Down if needed) — 2–3 × 30–40s/side

**Thoracic \+ Breathing Reset:**

* Open-Book Rotations — 2 × 8/side
* Child’s Pose — 2 minutes
* Box Breathing — 3 minutes (4-4-4-4)

Note: No hanging, no grip loading, no band pulldown work on Day 2\.

### **Day 3: Posterior Chain \+ Knee Health**

**Power Cleans (Block 1: 4 × 3 @ 52–60 kg)**

* Perform after Day 3 warm-up
* Technique focus: vertical bar path, fast turnover
* **RPE 6–7 only.** Add 2.5kg only when all reps smooth.

**The Workout**

* **A1: Romanian Deadlift (RDL):** 4 sets of 8 reps @ \~85kg.
* **B1: Goblet Squats:** 3 sets of 10 reps (Knee-friendly squat mechanics).
* **B2: Chest-Supported Row:** 3 sets of 10–12 reps.
* **C1: Face Pulls:** 3 sets of 15–20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side (Critical for hip flexors).
* **Light Weighted Jefferson Curl:** 2 minutes (Spinal segmentation).
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity \+ Push (Workout B)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:**
  * 20 Band Pull-Aparts
  * 15 Band External Rotations (per side)
  * 3 x 5 Scapular Pull-ups (3s Hold)
  * 20–30s Passive Bar Hang
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).
* **Skill Practice (HSPU Volume):** 3 sets of 3 reps.

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 3 sets of 3–5 reps @ \+10kg. (Use Double Progression).
* **B1: Dips:** 3 sets of 6–8 reps (Bodyweight, Tempo 2-1-1-0).

**Part 2: Accessories (10-Minute Hard Cap)**

* **Bulgarian Split Squat EMOM:** 2-minute EMOM — Minute 1 Left Leg, Minute 2 Right Leg (Run 3 rounds \= 6 minutes).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side (Releasing tension from Dips).
* **Lat Prayer (Bench/Box):** 60s (Releasing Lats).
* **Standing Quad Stretch:** 60s per side (Releasing Quads).

## **WEEK 2**

**Summary:** Increasing metabolic accumulation; lower intensity on core.

### **Day 1: Pull Volume (Workout A)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).
* **Skill Practice (Crow Stand):** 3 sets of 10–20 seconds.

**The Workout**

* **A1: Pull-Up Ladders:** 5 sets of (1, 2, 3 reps) – Total 30 reps.
* **B1: Ring Rows:** 3 sets of 10 reps (Tempo 2-1-1-0).
* **B2: Incline Push-Ups:** 3 sets of 10 reps (Tempo 2-1-1-0).
* **C1: Hollow Rocks:** 3 sets of 20–25 seconds.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Anti-Rotation \+ Ankles (Template \+ Easier Core)**

* Run the revised standard Day 2 template from Week 1\.
* Swap Side Plank for **Half-Side Plank — 2 × 25–30s/side**.
* All other elements stay identical to the template.

### **Day 3: Posterior Chain \+ Knee Health**

**Power Cleans (Block 1: 4 × 3 @ 52–60 kg)**

* Perform after Day 3 warm-up
* Technique focus: vertical bar path, fast turnover
* **RPE 6–7 only.** Add 2.5kg only when all reps smooth.

**The Workout**

* **A1: Romanian Deadlift (RDL):** 4 sets of 8 reps @ \~85kg.
* **B1: Cyclist Squats (Heels-Elevated):** 3 sets of 10 reps.
* **B2: Chest-Supported Row:** 3 sets of 10–12 reps.
* **C1: Face Pulls:** 3 sets of 15–20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity \+ Push (Workout B)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).
* **Skill Practice (HSPU Volume):** 3 sets of 3 reps.

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 2 sets of 4 reps @ \+10–12.5kg. (Total 8 reps).
* **B1: Dips:** 3 sets of 6–8 reps (Bodyweight, Tempo 2-1-1-0).

**Part 2: Accessories (10-Minute Hard Cap)**

* **Bulgarian Split Squat EMOM:** 2-minute EMOM — Minute 1 Left Leg, Minute 2 Right Leg (Run 3 rounds \= 6 minutes).
* **Dead Bugs:** 2–3 sets (Core / anti-extension).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 3**

**Summary:** Peak volume for the block; focus on perfect ladder execution.

### **Day 1: Pull Volume (Workout A)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).
* **Skill Practice (Crow Stand):** 3 sets of 10–20 seconds.

**The Workout**

* **A1: Pull-Up Ladders:** 5 sets of (1, 2, 3 reps) – Total 30 reps.
* **B1: Ring Rows:** 3 sets of 10 reps.
* **B2: Incline Push-Ups:** 3 sets of 10 reps.
* **C1: Hollow Rocks:** 3 sets of 20–25 seconds.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Anti-Rotation \+ Ankles (Tissue Quality Emphasis)**

* Run the revised standard Day 2 template from Week 1\.
* Add **Standing Calf ISO Hold (mid-range) — 2 × 30s/side**.
* Everything else remains as templated.

### **Day 3: Posterior Chain \+ Knee Health**

**Power Cleans (Block 1: 4 × 3 @ 52–60 kg)**

* Perform after Day 3 warm-up
* Technique focus: vertical bar path, fast turnover
* **RPE 6–7 only.** Add 2.5kg only when all reps smooth.

**The Workout**

* **A1: Single-Leg Romanian Deadlift:** 3 sets of 8 reps per side.
* **B1: Goblet Squats:** 3 sets of 8 reps (Slightly heavier).
* **B2: Chest-Supported Row:** 3 sets of 10–12 reps.
* **C1: Face Pulls:** 3 sets of 15 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity \+ Push (Workout B)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).
* **Skill Practice (HSPU Volume):** 3 sets of 3 reps.

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 2 sets of 5 reps @ \+12.5kg (Only if Week 2 felt clean).
* **B1: Dips:** 3 sets of 6–8 reps (Bodyweight, Tempo 2-1-1-0).

**Part 2: Accessories (10-Minute Hard Cap)**

* **Chest-Supported Row:** 2 sets of 12 reps.
* **Bulgarian Split Squat EMOM:** 2-minute EMOM — Minute 1 Left Leg, Minute 2 Right Leg (Run 3 rounds \= 6 minutes).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 4 (DELOAD)**

**Summary:** System reset. Volume dropped by 40%. RPE 6 MAX.

### **Day 1: Pull Volume (Deload)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).
* **Skill Practice:** Crow Stand – 2 sets of 10s (Easy).

**The Workout**

* **A1: Pull-Up Ladders:** 3 sets of (1, 2, 3 reps).
* **B1: Ring Rows:** 2 sets of 10 reps (Easy).
* **B2: Incline Push-Ups:** 2 sets of 8 reps (Easy).
* **C1: Side Plank:** 2 sets (Recovery replacement for Hollow Rocks).

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery (Deload Version)**

* Run the revised standard Day 2 template at **\~70% volume**.
* Reduce all holds by **\~10–15 seconds**.
* Omit **Hip CARs** if the joints feel fatigued.

### **Day 3: Posterior Chain (Deload)**

**Power Cleans (Deload: 3 × 3 @ 40–50 kg)**

* Light technique focus, no grinding
* **RPE 5 max**

**The Workout**

* **A1: RDL:** 2 sets of 6 reps (Light weight).
* **B1: Goblet Squats:** 2 sets of 10 reps (**Deload Weight**).

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity (Deload)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 2 sets of 3 reps (**50% load / Easy**).
* **B1: Dips:** 2 sets (Easy).

**Part 2: Accessories**

* Skip Circuit or perform 1 round very lightly.

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

# **BLOCK 2: INTENSIFICATION (Weeks 5–8)**

**Block Objectives:** Density-based volume (EMOMs), manage hamstring fatigue, increase pushing capacity.

## **WEEK 5**

**Summary:** Moderate intensity start, EMOM volume introduction.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).
* **Skill Practice (Tuck Planche):** 3 sets of 6–8s Hold.

**The Workout**

* **A1: Pull-Up Density:** 32 total reps in 8 minutes (stay crisp; no grinders).
* **B1: Feet-Elevated Ring Rows:** 3 sets (Tempo 2-1-1-0).
* **B2: Parallette Push-Ups:** 3 sets.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles (Template \+ Adductor Support)**

* Run the revised standard Day 2 template from Week 1\.
* **Add Cossack Squat (Shallow Range) — 2 × 6/side**, slow and controlled for adductor support.
* Keep the session technical, not metabolic.

### **Day 3: Posterior Chain (Hamstring Focus)**

**Power Cleans (Block 2: 4 × 3 @ 52–60 kg)**

* Fast turnover, quiet feet, controlled catches
* **RPE 6–7.** Progression: add 2.5kg when all reps smooth

**The Workout**

* **A1: Romanian Deadlift (RDL):** 4 sets of 10 reps.
* **B1: Rower Leg Curls:** 3 sets of 10 reps (Replaces Sliders).
* **B2: Goblet Squats:** 3 sets of 10 reps.
* **C1: Back Extensions:** 3 sets of 12 reps.
* **D1: Weighted Glute Bridge:** 2 sets of 12 reps @ 20–30kg (Introduction).

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).
* **Skill Practice:** PPPU Lean – 3 sets of practice.

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 2 sets of 4 reps @ 10kg (8 reps total).
* **B1: Weighted Dips:** 3 sets (load \~5–10kg).

**Part 2: Accessories (10-Minute Hard Cap)**

* **Minute 1:** Bulgarian Split Squat (Left Leg) – 8 reps.
* **Minute 2:** Bulgarian Split Squat (Right Leg) – 8 reps.
* **Minute 3:** Weighted Hollow Rocks – 15 reps.

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 6**

**Summary:** Volume peak for EMOMs.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).
* **Skill Practice:** Tuck Planche – 3 sets of 6–8s.

**The Workout**

* **A1: Pull-Up EMOM:** 9 Minutes x 4 reps per minute (36 reps total).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 10 reps.
* **B2: Parallette Push-Ups:** 3 sets of 10 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles (Repeat Week 5 Structure)**

* Repeat Week 5: run the revised standard Day 2 template \+ **Cossack Squat (Shallow Range) — 2 × 6/side**.
* Stay technical; avoid turning this into conditioning.

### **Day 3: Posterior Chain**

**Daily Prep**

* **Ankle Mobility:** Knee-to-Wall Mobilization 2 min/side, Tibialis Raises 3 × 15–20 reps
* **Power Clean Warm-Up:** (See Global Protocols section)
  * Bar only: 5 Muscle Clean Pulls, 5 Hang High Pulls, 5 Hang Power Clean \+ Rack Pause
  * 3 × 3 @ 40–50kg (movement rehearsal)

**The Workout**

* **A1: Hang Power Cleans:** 4 sets of 3 reps @ **52–60 kg** (Block 1 Tech Focus).
  * Smooth pulls, vertical bar path, fast turnover. **RPE 6–7.** No grinding.
  * Add 2.5kg only when all reps are crisp.
* **B1: RDL:** 4 sets of 10 reps.
* **B2: Rower Leg Curls:** 3 sets of 10 reps.
* **B3: Goblet Squats:** 3 sets of 10 reps.
* **C1: Back Extensions:** 3 sets of 12 reps.
* **D1: Weighted Glute Bridge:** 3 sets of 12 reps @ 20–30kg (Peak volume).

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).
* **Skill Practice:** PPPU Lean.

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 1 set of 4 reps (**Heavy single set** @ \~5–10kg).
* **B1: Weighted Dips:** 3 sets of 8 reps.

**Part 2: Accessories (10-Minute Hard Cap)**

* **Minute 1:** Bulgarian Split Squat (Left Leg) – 8 reps.
* **Minute 2:** Bulgarian Split Squat (Right Leg) – 8 reps.
* **Minute 3:** Dead Bugs – easy pace (share the 3-min EMOM with BSS).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 7**

**Summary:** Intensity Peak. Shifting to triples for speed.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).
* **Skill Practice:** Tuck Planche – 3 sets of 6–8s.

**The Workout**

* **A1: Pull-Up Density:** 30 total reps in 10 minutes (intensity peak).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 10 reps.
* **B2: Parallette Push-Ups:** 3 sets of 10 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles (Fatigue Management Week)**

* Run the revised standard Day 2 template **without** the Cossack add-on this week.
* Extend **Child’s Pose to 3 minutes**.
* Keep all other template elements unchanged.

### **Day 3: Posterior Chain**

**Power Cleans (Block 1: 4 × 3 @ 52–60 kg)**

* Perform after Day 3 warm-up
* Smooth pulls, vertical bar path, no grinding
* **RPE 6–7 only.** Add 2.5kg only when all reps smooth.

**The Workout**

* **A1: RDL:** 4 sets of 10 reps.
* **B1: Rower Leg Curls:** 3 sets of 10 reps.
* **B2: Goblet Squats:** 3 sets of 10 reps.
* **C1: Back Extensions:** 3 sets of 12 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).
* **Skill Practice:** PPPU Lean.

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 3 sets of 3 reps (**Heavy**).
* **B1: Weighted Dips:** 3 sets of 5 reps.

**Part 2: Accessories (10-Minute Hard Cap)**

* **Minute 1:** Bulgarian Split Squat (Left Leg) – 8 reps.
* **Minute 2:** Bulgarian Split Squat (Right Leg) – 8 reps.
* **Minute 3:** Weighted V-Ups – 3 sets of 12 reps (run as EMOM with the BSS pair).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 8 (DELOAD)**

**Summary:** Standard deload. Focus on mobility. RPE 6 MAX.

### **Day 1: Pull Volume (Deload)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).
* **Skill Practice:** Tuck Planche (Easy practice).

**The Workout**

* **A1: Pull-Up Density:** 18 total reps in 6 minutes (RPE 6 max).
* **B1: Feet-Elevated Ring Rows:** 2 sets of 8 reps.
* **B2: Parallette Push-Ups:** 2 sets of 8 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery (Deload)**

* Run the revised standard Day 2 template at **60–70% volume**.
* No added work; intent is a **nervous system down-shift**.

### **Day 3: Posterior Chain (Deload)**

**Power Cleans (Deload: 3 × 3 @ 40–50 kg)**

* Light technique focus, no grinding
* **RPE 5 max**

**The Workout**

* **A1: RDL:** 3 sets of 8 reps (Light).
* **B1: Rower Leg Curls:** 2 sets of 8 reps.
* **B2: Goblet Squats:** 2 sets of 8 reps.
* **C1: Back Extensions:** 2 sets of 10 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity (Deload)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).

**Part 1: Primary Lifts**

* **A1: Weighted Pull-Ups:** 2 sets of 3 reps (**Easy**).
* **B1: Weighted Dips:** 2 sets of 5 reps (Easy).

**Part 2: Accessories**

* Skip Circuit or perform 1 round very lightly.

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

# **BLOCK 3: NEUTRAL GRIP INTEGRATION (Weeks 9–12)**

**Block Objectives:** Protect distal biceps tendon (Neutral Grip Mandate), develop unilateral stability.

## **WEEK 9**

**Summary:** Introduction to Clusters and Neutral Grip pulling.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).
* **Skill Practice:** None listed (Focus on Cluster setup).

**The Workout**

* **A1: Cluster Pull-ups:** 4 sets of (3 reps, Rest 20s, 3 reps, Rest 20s, 3 reps).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 15 reps.
* **B2: Parallette Push-Ups:** 3 sets of 15 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles**

* Run the revised standard Day 2 template (unchanged).

### **Day 3: Posterior Chain \+ Lower Body Power**

**Power Cleans (Block 3: 5 × 2 @ 60–65 kg)**

* Fast bar speed, stable rack position, no early arm pull
* **Critical Rule:** If elbows feel irritated → skip cleans; substitute with Clean Pulls or High Pulls
* **RPE 6–7.** Zero missed reps.

**The Workout**

* **A1: Single Leg RDL:** 3 sets of 10 reps per side.
* **B1: Goblet Squats:** 3 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Neutral Grip (Parallel) Pull-ups (Weighted):** 2 sets of 3 reps.
* **B1: Pseudo Planche Push-Ups (PPPU):** 3 sets of 5–8 reps.
* **C1: Weighted Hollow Hold (with BSS EMOM):** 3-minute EMOM shared with BSS; hold for quality each minute.

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 10**

**Summary:** Elbow protection week. Isometric focus.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Cluster Pull-ups:** 4 sets of (4 reps, Rest 20s, 3 reps, Rest 20s, 3 reps).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 15 reps.
* **B2: Parallette Push-Ups:** 3 sets of 15 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles (Isometric Recovery Week)**

* Run the revised standard Day 2 template.
* Hold Side Planks **25–30s only**; prioritize smooth breathing throughout.

### **Day 3: Posterior Chain**

**Power Cleans (Block 3: 5 × 2 @ 60–65 kg)**

* Fast bar speed, stable rack position, no early arm pull
* **Critical Rule:** If elbows feel irritated → skip cleans; substitute with Clean Pulls or High Pulls
* **RPE 6–7.** Zero missed reps.

**The Workout**

* **A1: Single Leg RDL:** 3 sets of 10 reps per side.
* **B1: Goblet Squats:** 3 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity (Isometric Focus)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Ring Support Holds (slight RTO) OR Active Hang (Neutral):** 3 sets of 30 seconds.
* **B1: Chest Supported Row:** 2 sets of 12 reps.
* **Core (EMOM with BSS):** Dead Bugs – easy pace (3-min EMOM shared with BSS).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 11**

**Summary:** Return to Neutral Grip intensity.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Cluster Pull-ups:** 5 sets of (3 reps, Rest 20s, 3 reps, Rest 20s, 3 reps).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 15 reps (stay strict on elevation).
* **B2: Incline Push-Ups:** 3 sets of 15 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles**

* Run the revised standard Day 2 template.
* Ensure **Hip CARs 2 × 5/side** are included this week.

### **Day 3: Posterior Chain**

**Power Cleans (Block 3: 5 × 2 @ 60–65 kg)**

* Fast bar speed, stable rack position, no early arm pull
* **Critical Rule:** If elbows feel irritated → skip cleans; substitute with Clean Pulls or High Pulls
* **RPE 6–7.** Zero missed reps.

**The Workout**

* **A1: Single Leg RDL:** 3 sets of 10 reps per side (@ \~8–12kg).
* **B1: Goblet Squats:** 3 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Neutral Grip Pull-ups (Weighted):** 2 sets of 4 reps (**Heavy**).
* **B1: PPPU:** 3 sets of 5–8 reps.
* **Core:** Bulgarian Split Squats \+ Weighted V-Ups – 3-minute EMOM (alternate BSS left, BSS right, V-Ups).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 12 (CONNECTIVE TISSUE DELOAD)**

**Summary:** System reset with strict restrictions. NO Dead Hangs. NO Long Hollow Holds. NO Weighted Core.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, **5s Hang (Reduced)**.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Cluster Pull-ups:** 3 sets of (3 reps, 3 reps, 3 reps).
* **B1: Feet-Elevated Ring Rows:** 2 sets of 10 reps (Easy).
* **B2: Incline Push-Ups:** 2 sets of 10 reps (Easy).

**Cool Down (Reduced)**

* **Gentle stretching only.** No long hangs.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery (Connective Tissue Deload)**

* Run the revised standard Day 2 template at **50–60% volume**.
* No long holds, no deep ranges; **skip Side Planks** this week.

### **Day 3: Posterior Chain (Deload)**

**Power Cleans (Block 3 Deload: 2 × 2 @ 30–40 kg)**

* Light technique doubles, unweighted bar or very light
* **RPE 5 max.** No grinding. No missed reps.

**The Workout**

* **A1: RDL:** 2 sets of 6 reps (Very light).
* **B1: Goblet Squats:** 2 sets of 8 reps (Light, Deload).
* **C1: Face Pulls:** 2 sets of 12 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, **No Hang**.

**The Workout**

* **A1: Neutral Grip Pull-ups:** 2 sets of 3 reps (Unweighted or very light).
* **B1: PPPU:** 2 sets of 5 reps (Easy).
* **Core:** Tuck Planche Holds – 3 sets of 6 seconds max (Straight-Arm Cap, no hangs this day).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

# **BLOCK 4: ACCUMULATION (Weeks 13–16)**

**Block Objectives:** Peak absolute strength testing, challenge core strength under fatigue.

## **WEEK 13**

**Summary:** Re-introducing AMRAPs with strict technical caps.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:** 3 sets of (AMRAP minus 2 reps).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 15 reps.
* **B2: Incline Push-Ups:** 3 sets of 15 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles**

* Run the revised standard Day 2 template.
* **Optional (only if tissues feel excellent):** Marching Dead Bug — 2 × 8/side (low tension).

### **Day 3: Posterior Chain**

**Power Cleans (Block 4: 3 × (2-2) Cluster @ 65–70 kg)**

* Cluster format: 2 reps, 15–20s rest, 2 reps, full 2–3 min recovery between sets
* Quality exposures with neural intent
* **RPE 7–8.** Zero missed reps. Stop when catches aren't crisp.

**The Workout**

* **A1: RDL:** 4 sets of 8 reps (@ \~85kg).
* **B1: Goblet Squats:** 3 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Weighted Pull-ups:** 2 sets of 4 reps @ 14kg (Double Progression).
* **B1: Weighted Dips:** 3–4 sets (load \~5–10kg).
* **C1: Chest Supported Row:** 2 sets of 12 reps (Tempo 2-1-1-0).
* **Core:** Side Planks – 3 sets of 45s per side (**Recovery**).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 14**

**Summary:** Increasing intensity on AMRAPs and Dips.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:** 3 sets of (AMRAP minus 1 rep).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 15 reps.
* **B2: Incline Push-Ups:** 3 sets of 10 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles**

* Repeat Week 13’s revised standard Day 2 template.
* **Remove the Dead Bug** if elbows or hips feel taxed.

### **Day 3: Posterior Chain**

**Power Cleans (Block 4: 3 × (2-2) Cluster @ 65–70 kg)**

* Cluster format: 2 reps, 15–20s rest, 2 reps, full 2–3 min recovery between sets
* Quality over volume, maintain bar speed
* **RPE 7–8.** Zero missed reps.

**The Workout**

* **A1: RDL:** 4 sets of 8 reps.
* **B1: Goblet Squats:** 3 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Weighted Pull-ups:** 1 set of 4 reps @ 16kg (Work up to 16kg if earned).
* **B1: Weighted Dips:** 3–4 sets (load \~5–10kg).
* **C1: Chest Supported Row:** 2 sets of 12 reps.
* **Core:** Optional only if fresh (no required core this week).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 15**

**Summary:** Peak Volume & Load. All-out effort week.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:**
  * Set 1: All-out AMRAP.
  * Set 2: Sub-max effort (leave 2 in reserve).
  * Set 3: Sub-max effort (leave 2 in reserve).
* **B1: Feet-Elevated Ring Rows:** 3 sets of 15 reps.
* **B2: Incline Push-Ups:** 3 sets of 10 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles (Peak Stress Week)**

* Run the revised standard Day 2 template only.
* Shorten **Side Planks to 25–30s** this week.

### **Day 3: Posterior Chain**

**Power Cleans (Block 4: 3 × (2-2) Cluster @ 65–70 kg)**

* Cluster format: 2 reps, 15–20s rest, 2 reps, full 2–3 min recovery between sets
* This is the final heavy week — quality only
* **RPE 7–8.** Zero missed reps. Stop if form breaks.

**The Workout**

* **A1: RDL:** 4 sets of 8 reps.
* **B1: Weighted Glute Bridge:** 3 sets of 12 reps.
* **B2: Goblet Squats:** 3 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 4×12 @ same band, RPE 7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Weighted Pull-ups:** Work up to 17.5kg (PR Attempt).
* **B1: Weighted Dips:** 3–4 sets (load \~5–10kg).
* **C1: Chest Supported Row:** 2 sets of 12 reps.
* **Core:** Side Planks – 3 sets of 45s per side (**Recovery**).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 16 (DELOAD)**

**Summary:** Significant volume cut before the taper. RPE 6 MAX.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:** 2 sets of (AMRAP minus 4 reps).
* **B1: Ring Rows:** 2 sets of 10 reps (Easy).
* **B2: Incline Push-Ups:** 2 sets of 8 reps (Easy).

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery (Deload)**

* Run the revised standard Day 2 template at **\~60% volume**.
* Extend the breathing block to **4–5 minutes**.

### **Day 3: Posterior Chain**

**Power Cleans (Block 4 Deload: Clean Pulls @ 70–85 kg)**

* Pull only, no catching — build pulling strength
* 3 sets of 3 reps, full power, controlled lower
* **RPE 6–7.** Focus on bar path and acceleration

**The Workout**

* **A1: RDL:** 3 sets of 6 reps (Light).
* **B1: Goblet Squats:** 2 sets of 10 reps (Light).
* **C1: Face Pulls:** 2 sets of 15 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Weighted Pull-ups:** 2 sets of 3 reps (Easy).
* **B1: Weighted Dips:** 2 sets (Easy).
* **Accessories:** Skip rows/core to keep it light.

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

# **BLOCK 5: PEAK & TAPER (Weeks 17–20)**

**Block Objectives:** Sharpen neural drive, minimize joint stress, full deload before test.

## **WEEK 17**

**Summary:** High intensity, reduced joint stress variations.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:** 3 sets of (AMRAP minus 1 rep).
* **B1: Feet-on-Floor Rows:** 3 sets of 10 reps (Tempo 3-1-1-0).
* **B2: Incline Push-Ups:** 3 sets of 10 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery \+ Ankles**

* Run the revised standard Day 2 template at **\~80% volume**.
* Prioritize slow, breath-led movement quality.

### **Day 3: Posterior Chain**

**Power Cleans (Block 5 \- Week 17: 3 × 2 @ 35–45 kg Light Hang Power Cleans)**

* Light technical work, controlled catches, no grinding
* **RPE 5 max.** Stop if fatigue appears.

**The Workout**

* **A1: RDL:** 4 sets of 8 reps.
* **B1: Weighted Glute Bridge:** 3 sets of 12 reps.
* **B2: Goblet Squats:** 3 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 20 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity (Archer Focus)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×12 @ easy band, RPE 6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Archer Negatives:** 4 sets of 2 reps per side.
  * *Tempo: 5–6 second eccentric. NO pause at bottom.*
* **Core:** Weighted V-Ups – 3 sets of 10 reps (**Hard**).

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 18**

**Summary:** Tapering \+ Archer Negative Peak.

### **Day 1: Pull Volume**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:** 2 sets of (AMRAP minus 2 reps). (**Volume Reduced by 20%**).
* **B1: Feet-on-Floor Rows:** 2 sets of 10 reps.
* **B2: Incline Push-Ups:** 2 sets of 10 reps.

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery**

* Run the revised standard Day 2 template at **\~70% volume**.
* Side Plank **20–25s only**; keep the rest recovery-focused.

### **Day 3: Posterior Chain**

**Power Cleans (Block 5 \- Week 18: 2 × 2 @ 30–40 kg Minimal Power Work)**

* Very light double, movement rehearsal only
* **RPE 4–5.** No grinding, no fatigue.

**The Workout**

* **A1: RDL:** 3 sets of 6 reps.
* **B1: Weighted Glute Bridge:** 2 sets of 10 reps.
* **B2: Goblet Squats:** 2 sets of 10 reps.
* **C1: Face Pulls:** 3 sets of 15 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Pull Intensity**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 3×15 @ same band, RPE 6–7 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Archer Negatives:** 5 sets of 3 reps per side.
  * *Tempo: 5–6 second eccentric.*
* **Core:** Weighted V-Ups – 2 sets of 10 reps.

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 19 (FULL DELOAD)**

**Summary:** The calm before the test. 50% volume. Very Low Intensity.

### **Day 1: Pull Volume (Deload)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:** 2 sets of (50% of your max reps).
* **B1: Rows:** 2 sets of 8 reps (Easy).
* **B2: Push-Ups:** 2 sets of 8 reps (Easy).

**Cool Down (Volume Reset)**

* **Passive Dead Hang:** 30–60s.
* **Butcher’s Block Stretch:** 60s.
* **Cobra Stretch:** 60s.
* **Wrist Extensor Stretch:** 30s per side.

### **Day 2: Recovery (Full Deload)**

* Very light mobility only; remove **Pallof Press** and any added core/stability loading.
* Focus on breathing plus gentle hips/ankles work.

### **Day 3: Posterior Chain (Deload)**

**Power Cleans (Block 5 \- Week 19: 2 × 1 @ 40–50 kg Light Singles)**

* Light singles, comfortable weight, no grind
* **RPE 4 max.** Smooth and easy.

**The Workout**

* **A1: RDL:** 2 sets of 6 reps (Very Light).
* **B1: Goblet Squats:** 2 sets of 8 reps (Very Light).
* **C1: Face Pulls:** 2 sets of 15 reps.

**Cool Down (Post-Hinge)**

* **Couch Stretch:** 90s per side.
* **Light Weighted Jefferson Curl:** 2 minutes.
* **90/90 Hip Rotations:** 10 reps per side.

### **Day 5: Prime (Deload)**

**Daily Prep**

* **Rower Protocol:** Minute 1–2 Easy, Minute 3–4 Moderate, Minute 5 (3 x 10s Sprints).
* **Tendon Prep:** 20 Band Pull-Aparts, 15 Ext Rotations/side, 3x5 Scap Pull-ups, 30s Hang.
* **Straight-Arm Band Pulldowns:** 2–3×12 (deload), RPE 5–6 (See Global Protocols for progression schedule).

**The Workout**

* **A1: Pull-ups:** 2 sets of 3 reps (Bodyweight).
* **A2: Push-ups:** 2 sets of 5 reps.
* **Core:** None.

**Cool Down (Anterior Chain Openers)**

* **Banded Pec Stretch:** 60s per side.
* **Lat Prayer:** 60s.
* **Standing Quad Stretch:** 60s per side.

## **WEEK 20 (TEST WEEK)**

**Summary:** The Final Exam.

### **Day 1: Endurance Test**

**Battery Conservation Warm-up**

* 5 mins Rower (Easy).
* 10 Scap Pulls.
* 3 Pull-ups @ 50% effort.
* 1 Pull-up @ 75% effort.
* **Rest exactly 4 minutes.**

**The Test**

* **A1: Max Reps Pull-Ups (Bodyweight).**
* *Rest 5 minutes.*
* **B1: Max Reps Dips (Bodyweight).**

**Cool Down**

* Full Body Foam Roll.

### **Day 2–4: Rest & Mobility**

* Optional light mobility only; **no core or stability loading**.
* Keep joints warm, not trained (walk, gentle stretch, easy breath work).

### **Day 5: Strength Test**

**Battery Conservation Warm-up**

* 5 mins Rower (Easy).
* 10 Scap Pulls.
* Warm-up sets for Pull-up and Push-up (Singles increasing in weight).

**The Test**

* **A1: 3-Rep Max (3RM) Weighted Pull-Up.**
* *Rest 5 minutes.*
* **B1: 5-Rep Max (5RM) Weighted Push-Up (or Dip if preferred).**
* *Rest 5 minutes.*
* **C1: Max Quality Archer Negatives.**

**Conclusion**

* **Week 21:** Post-Program Reload (60% volume of Week 1, NO Weighted Dips/Pulls).

