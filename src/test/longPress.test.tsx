...
describe('useLongPress', () => {
  ...
  it('should cancel the previous pending press when a new press starts', () => {
    const callback = vi.fn();
    const { result } = renderHook(() => useLongPress(callback));
    ...
    act(() => {
      result.current.onMouseDown({ target: firstTarget });
    });
    ...
    act(() => {
      result.current.onMouseDown({ target: secondTarget });
    });
    ...
    expect(callback).not.toHaveBeenCalled();
    expect(firstTarget.removeEventListener).toHaveBeenCalledWith(
      'contextmenu',
      expect.any(Function)
    );
  ...
  it('should clear pending press work on unmount', () => {
    const callback = vi.fn();
    ...
    expect(callback).not.toHaveBeenCalled();
    expect(target.removeEventListener).toHaveBeenCalledWith('contextmenu', expect.any(Function));
  });
});
