# Power Clean / Bench Hybrid Block
...
- bench: heavy working set of 4-5 at RPE 8, then 1-2 backoff sets
- clean: clusters or crisp singles
- squat: heavy working set of 3-4 at RPE 8, then 1-2 backoff sets
...
- bench: top set of 2-3, then 1 backoff set
- clean: 3-5 quality singles
- squat: top set of 2-3, then 1 backoff set only if speed supports it
...
- easy and long runs: 6:40-7:20/km
- recovery shuffle: 6:50-7:30/km
- Fartlek easy jog: 5:55-6:25/km in accumulation, then 5:40-6:05/km as fitness improves
...
- cut backoff bench volume before adding substitute pressing
- replace weighted pull-ups with chest-supported rowing when vertical pulling is the irritant
- avoid combining hard benching and hard chinning when symptoms are elevated
...
- move from doubles to singles when turnover or rack quality drops twice in one session
